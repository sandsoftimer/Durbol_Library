...
public class DurbolConfigarator : AssetPostprocessor
{
    ...
    private static void SetDurbolDefines()
    {
        var filePastLocation = Application.dataPath + "/csc.rsp";

        string[,] sdks = new string[,]
        {
            { "Adjust", "DURBOL_ADJUST_SDK_INSTALLED"},
            { "FacebookSDK", "DURBOL_FACEBOOK_SDK_INSTALLED"},
            { "GameAnalytics","DURBOL_GAMEANALYTICS_SDK_INSTALLED"},
            { "Lion Studios","DURBOL_LIONSTUDIO_SDK_INSTALLED"},
            { "PotatoSDK","POTATO_SDK_INSTALLED"},
            { "ByteBrewSDK", "DURBOL_BYTE_BREW_SDK_INSTALLED"},
            { "SupersonicWisdom", "SUPERSONIC_WISDOM_SDK_INSTALLED"},
            { "Plugins/Demigiant", "DOTWEEN"},
            { "Homa Games", "HOMA_SDK_INSTALLED"}
        };
    ...
    private static void SetDurbolScriptTemplates()
    {
        CreateDirectoryToProject("ScriptTemplates");

        var fileCopyLocation = Application.dataPath + "/Durbol_Library/Scripts/Auto_Durbol_Configs/DurbolScriptTemplates/81-C# Script (Durbol)__DurbolBehaviour-NewDurbolBehaviour.cs.txt";
        var filePastLocation = Application.dataPath + "/ScriptTemplates/81-C# Script (Durbol)__DurbolBehaviour-NewDurbolBehaviour.cs.txt";
        if (!File.Exists(filePastLocation))
        {
            File.Copy(fileCopyLocation, filePastLocation);
        }

        fileCopyLocation = Application.dataPath + "/Durbol_Library/Scripts/Auto_Durbol_Configs/DurbolScriptTemplates/82-C# Script (Durbol)__DurbolManager-NewDurbolManager.cs.txt";
        filePastLocation = Application.dataPath + "/ScriptTemplates/82-C# Script (Durbol)__DurbolManager-NewDurbolManager.cs.txt";
        if (!File.Exists(filePastLocation))
        {
            File.Copy(fileCopyLocation, filePastLocation);
        }
    ...
    private static void SetEditorBuildSettings()
    {
        ...
        for (int i = 0; i < sceneCount; i++)
        {
            ...
            if (scenesPath[i].Contains("Durbol_BootScene"))
            {
                bootSceneIndex = i;
            }
        }
        ...
        if (bootSceneIndex == -1)
        {
            ...
            Debug.LogError("<Color=red>Durbol_BootScene was not found in BuildSettings.</Color> <Color=green>Re-Added at 0 index</Color>. To modify/add double click me.");
            flag = true;
            string bootScenePath = "Assets/DurbolLibrary/Scenes/Durbol_BootScene.unity";
